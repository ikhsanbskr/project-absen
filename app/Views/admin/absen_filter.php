        ...
        <div class="card">
            <div class="card-body">
                <div class="text-center">
                    <h6>LAPORAN KEHADIRAN <?= $nama == '' ? 'SEMUA KARYAWAN' : strtoupper($nama) ?></h6>
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6 text-right mb-2">
                        <?php if ($nama == '') : ?>
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                    <button class="dropdown-item" type="submit" form="laporan-pdf" onclick="this.form.target='_blank'; return true;">Laporan Absen</button>
                                    <button class="dropdown-item" type="submit" form="summary-pdf" onclick="this.form.target='_blank'; return true;">Summary Absen</button>
                                </div>
                            </div>
                        <?php endif ?>
                        <?php if ($nama != '') : ?>
                            <form action="/absen/filter/laporan" method="POST" formtarget="_blank">
                        ...
                        <?php endif ?>
                        <form action="/absen/filter/laporan-semua" method="POST" id="laporan-pdf">
                            <input type="hidden" name="min-tanggal" value="<?= $min ?>">
                            <input type="hidden" name="max-tanggal" value="<?= $max ?>">
                            <input type="hidden" name="nama-karyawan" value="<?= $nama ?>">
                        </form>
                        <form action="/absen/filter/summary" method="POST" id="summary-pdf">
                            <input type="hidden" name="min-tanggal" value="<?= $min ?>">
                            <input type="hidden" name="max-tanggal" value="<?= $max ?>">
                            <input type="hidden" name="nama-karyawan" value="<?= $nama ?>">
                        </form>
